...
MergeIterator::MergeIterator(MemTableIterator it_a, SstIterator it_b)
...
std::shared_ptr<BaseIterator> MergeIterator::operator++(int) {
  auto tmp = std::make_shared<MergeIterator>(*this);
  ++(*this);
  return tmp;
}
